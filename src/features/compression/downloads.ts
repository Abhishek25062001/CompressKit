...
export function downloadItem(id: string): void {
...
export async function downloadAll(onProgress?: (ratio: number) => void): Promise<void> {
  const { order, items } = useQueueStore.getState();
  const results = order
    .map((id) => items[id])
  ...
  downloadBlob(zip, `compresskit-${stamp}.zip`);
}
